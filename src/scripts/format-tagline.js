(function (DR, document) {
    ...
    DR.addListener('rolesidentified', function (roles) {
        requestAnimationFrame(function () {
            document.querySelectorAll('.thing.self > .entry .tagline, .thing.comment > .entry .tagline').forEach(taglineFormatter);
        });

        function computeLocalTime() {
            let here = new Date(),
                there = new Date(here.getTime() + (here.getTimezoneOffset() + (this.dataset.offset * 60)) * 60 * 1000);

            this.title = "Right now it's " + there.toLocaleString() + ' in their time zone.';
        }

        function taglineFormatter(tagline) {
            if (tagline.classList.contains(CLASSNAMES.COMMENT_TAGLINE))
                return;
