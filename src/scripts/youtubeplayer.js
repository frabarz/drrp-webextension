(function() {
	...
	var v,
		...
		code,
		startTime,
		vids = document.querySelectorAll(
	...
	for (v = 0; v < vids.length; v++) {
		...
		if (code) {
			...
			if (startTime) {
				...
				if (startTime[1])
					startTime =
						3600 * parseInt(startTime[1]) +
						60 * parseInt(startTime[2]) +
						parseInt(startTime[3]);
				else if (startTime[2])
			...
			vid.parentNode.insertBefore(
				createPlayer(code, startTime),
				vid.nextSibling
			);
		}
	}
