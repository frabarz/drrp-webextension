(function(DR) {
	...
	if (document.title.toLowerCase().indexOf("signup") == -1) return;

	var trials = [];

	trials[0] = DR.getThreadSource().then(currentExtractor);

	trials[1] = DR.fetch(
		"GET",
		"https://frbrz-kumo.appspot.com/postit/participation.json"
	).then(historyCruncher);

	Promise.all(trials).then(priorityCalculator).then(console.log.bind(console));
	...
	function currentExtractor(source) {
		...
		return source[1].data.children
			.filter(function(reply) {
				return seen.hasOwnProperty(reply.data.author)
					? false
					: (seen[reply.data.author] = true);
			})
			.sort(function(a, b) {
	...
	function priorityCalculator(result) {
		...
		return Object.keys(result[0])
			.map(function(userkey) {
				...
				user.hash = [100 + user.order + user.counter, 100 + user.counter]
					.map(leadingZero)
					.join("-");
			...
			.map(function(s) {
				return (
					[s.order, s.main, s.reserve].map(leadingZero).join(" ") +
					" " +
					s.hash +
					" " +
					s.username
